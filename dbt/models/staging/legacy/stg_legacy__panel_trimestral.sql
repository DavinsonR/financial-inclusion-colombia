{{ config(materialized='view', enabled=(target.type == 'duckdb')) }}
...
-- Solo existe en DuckDB: lee el Parquet congelado con read_parquet_path(), que no tiene equivalente en
-- BigQuery ni en Snowflake; allí el modelo (y sus pruebas) quedan deshabilitados en vez de romper la compilación.
...
select
    cast("Depto Base"                    as {{ dbt.type_string() }})          as departamento,
    ...
    cast("MONTO TOTAL CREDITO CONSUMO"   as {{ type_double() }})           as monto_total_cred_cons,
    cast("MONTO TOTAL CREDITO VIVIENDA"  as {{ type_double() }})           as monto_total_cred_viv,
    cast("MONTO TOTAL MICROCREDITO"      as {{ type_double() }})           as monto_total_micro,

    -- Controles anuales repetidos por trimestre (DANE, MinTIC)
    cast("Poblacion"                     as {{ type_double() }})           as poblacion,
    cast("Conexión a internet %"         as {{ type_double() }})           as internet_pct,
    cast("Años promedio de educación"    as {{ type_double() }})           as educacion_anios,
    cast("IPC %"                         as {{ type_double() }})           as ipc_pct,
    cast("Empleo Formal"                 as {{ type_double() }})           as empleo_formal,
    cast("Empleo Informal"               as {{ type_double() }})           as empleo_informal,
    cast("Densidad Poblacional"          as {{ type_double() }})           as densidad_poblacional,
    cast("PIB Percapita"                 as {{ type_double() }})           as pib_percapita,
    cast("PIB Crecimiento"               as {{ type_double() }})           as pib_crecimiento
