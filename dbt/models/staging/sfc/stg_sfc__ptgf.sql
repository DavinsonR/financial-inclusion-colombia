...
-- Tabla legada de inclusión financiera de la SFC (2017Q4 a 2021Q1), con claves resueltas:
...
--   * tipo_id por la semilla map_sfc_tipo; las columnas numéricas son las 78 que registra map_sfc_columns
...
{% if execute %}
    {% set cols = run_query(
        "select distinct columna from " ~ ref('map_sfc_columns') ~ " where fuente = 'ptgf' order by 1"
    ).columns[0].values() %}
{% endif %}

with fuente as (

    select * from {{ source('sfc', 'sfc_ptgf_ywrb') }}

),

xw as (

    select unicap, dpto_ccdgo from {{ ref('xw_sfc_departamento') }} where fuente = 'ptgf'

),

tipos as (

    select tipo_raw, tipo_id from {{ ref('map_sfc_tipo') }} where fuente = 'ptgf'
...
base as (
    ...
    select
        cast(f.fechacorte as date)                  as fecha_corte,
        cast(f.unicap as integer)                   as unicap,
        cast(f.renglon as integer)                  as renglon,
        f.departamento                              as nombre_unidad_raw,
        f.municipio                                 as nombre_renglon_raw,
        coalesce(f.tipo, 'N/A')                     as tipo_raw,
        cast(f.tipoentidad as integer)              as tipo_entidad,
        cast(f.codigoentidad as integer)            as codigo_entidad,
        f.nombreentidad                             as nombre_entidad,
        f.pull_id,
        {% for c in cols %}
