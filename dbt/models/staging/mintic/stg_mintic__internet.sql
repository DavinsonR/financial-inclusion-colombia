...
select
    ...
    cast(anno as {{ dbt.type_string() }}) || 'Q' || cast(trimestre as {{ dbt.type_string() }})  as periodo_id,
    lpad(cast(cod_departamento as {{ dbt.type_string() }}), 2, '0')             as dpto_ccdgo,
    lpad(cast(cod_municipio as {{ dbt.type_string() }}), 5, '0')                as mpio_ccdgo,
    proveedor,
    segmento,
    tecnologia,
    cast(velocidad_bajada as {{ type_double() }})                            as velocidad_bajada_mbps,
    cast(velocidad_subida as {{ type_double() }})                            as velocidad_subida_mbps,
